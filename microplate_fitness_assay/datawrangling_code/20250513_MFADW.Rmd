...
#FEP24, Fitness Effects of Phage in 1st 24 hours
#Isolate 24h post-infection
OD.long24<-OD.long[(1:27744),]
OD.long24 <- OD.long24 %>%
  group_by(Well, Media, Strain, Infection) %>%
  summarize(total.OD = sum(OD600, na.rm = TRUE), .groups = "drop")
# ctrl avgs from OD.long24
ctrl_avg24 <- OD.long24 %>%
  filter(Infection == "Ctrl") %>%  # Keep only control rows
  group_by(Media, Strain) %>%
  summarize(ctrl_avg24 = mean(total.OD, na.rm = TRUE), .groups = "drop")
# subtract control average
FEP24 <- OD.long24 %>%
  left_join(ctrl_avg24, by = c("Media", "Strain")) %>%
  mutate(total.OD = ctrl_avg24 - total.OD) %>%
  select(-ctrl_avg24)  # Remove temporary column
FEP24<-FEP24%>%  #Fitness effects of Phage
  filter(!grepl("Ctrl", Infection))
```
...
#Infection vs no infection 24h--is there a diff vs 48h?
FEP24a <- aov(total.OD ~  Strain * Infection , data = FEP24)
summary(FEP24a)
